import { resolveSubject, parseGrade, assertSpecificHighSchoolMath } from './GenerateWorksheet.js';
...
export class ComposeWorksheet {
  ...
  async execute({
    ...
    const standards = await this.#resolveStandards({ school, grade, spec, topic, codes, limit, subject });
  ...
  async #resolveStandards({ school, grade, spec, topic, codes, limit, subject }) {
    let found;
