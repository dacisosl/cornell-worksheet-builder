...
const SUBJECT_REGISTRY = {
  ...
  // 수학 계열 — CSV 과목명(공통수학1, 미적분Ⅰ 등)은 label 부분일치로 조회된다.
  '수학': { template: 'math', label: '수학' },
  '공통수학1': { template: 'math', label: '공통수학1' },
  '공통수학2': { template: 'math', label: '공통수학2' },
  '미적분1': { template: 'math', label: '미적분Ⅰ' },
  '미적분Ⅰ': { template: 'math', label: '미적분Ⅰ' },
  '미적분2': { template: 'math', label: '미적분Ⅱ' },
  '미적분Ⅱ': { template: 'math', label: '미적분Ⅱ' },
  '확률과통계': { template: 'math', label: '확률과 통계' },
  '확률과 통계': { template: 'math', label: '확률과 통계' },
  '기하': { template: 'math', label: '기하' },
  '대수': { template: 'math', label: '대수' },
  '경제수학': { template: 'math', label: '경제 수학' },
  '경제 수학': { template: 'math', label: '경제 수학' },
};
...
/**
 * 고등학교의 일반 "수학" 자동 검색은 공통수학과 선택과목을 부분일치로 함께 가져오므로
 * 조용히 진행하지 않는다. 교사가 과목을 구체화하거나 성취기준 코드를 직접 고르면 허용한다.
 */
export function assertSpecificHighSchoolMath({ school, subject, codes }) {
  if (school !== '고등학교' || String(subject).trim() !== '수학') return;
  if (Array.isArray(codes) && codes.length > 0) return;
  throw new Error(
    '고등학교 수학은 과목을 구체적으로 지정해야 합니다. ' +
    '예: 공통수학1, 공통수학2, 대수, 미적분Ⅰ, 확률과 통계, 기하, 경제 수학. ' +
    '--standards <코드,..>로 성취기준을 직접 지정할 수도 있습니다.',
  );
}
...
export class GenerateWorksheet {
  ...
  async execute({
    ...
    assertSpecificHighSchoolMath({ school, subject, codes });
    ...
    if (Array.isArray(codes) && codes.length > 0) {
      ...
      found = await this.curriculum.search({ school, grade, subject: spec.label, keyword: topic, limit });
    }
