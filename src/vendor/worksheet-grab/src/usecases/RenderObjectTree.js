...
export class RenderObjectTree {
  ...
  execute(document, assets, meta = {}, opts = {}) {
    ...
    const html = buildDocumentHtml({
      ...
      moodFramesCss: assets.moodFramesCss || '',
      moodName: assets.moodName || '',
      // 자기완결(외부 URL 0개) 자산 — 호출부(loadRenderAssets)가 조달해 넘긴다.
      // 미주입이면 종전 CDN 경로 그대로라 무회귀다.
      selfContained: assets.selfContained ?? null,
    });
...
// export 하는 이유(PPTX 기하, 2026-08-10): 인쇄 페이지 본문은 flow 개체별 이 함수 산출의
// join 이다(아래 :109 의 flowHtml). 따라서 개체별로 같은 ctx 로 이 함수를 호출하면 각 개체가
// 인쇄 HTML 에서 차지하는 **바이트 구간이 결정적으로 확정된다**. PPTX 좌표를 얻으려면 개체
// 경계 표식이 필요한데 `data-oid` 는 editMode 전용이고, editMode 렌더는 `.wg-obj` 가
// `.std-box + .std-box` 같은 인접 형제 결합자를 끊어 인쇄와 기하가 어긋난다(실측 7.57px,
// 허용오차의 15배 — test/render/pptx-geometry.render.test.js). 그래서 인쇄 렌더에 표식만
// 후주입하는 경로가 필요하고, 그 경계를 아는 유일한 방법이 이 함수다.
// **본문 로직은 한 줄도 바뀌지 않았다**(export 키워드만 추가).
export function renderFlowObject(obj, ctx) {
